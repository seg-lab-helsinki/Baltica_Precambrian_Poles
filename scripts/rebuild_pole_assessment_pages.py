...
# Web-table order. This now matches Ahmed's Excel order.
PREFERRED_COLUMNS = [
    "site",
    "dir_dec",
    "dir_inc",
...
def build_site_level_figure_cards(sheet_name):
    ...
    vgp_rel = f"../figures/site_level_examples/{slug}/vgp_distribution.png"
...
<section class="card" id="site-level-vgp-distribution">
  <h2>Site-level VGP distribution</h2>
  <p>
    Site-level VGPs show the pole scatter behind the published mean pole.
    This plot is the basis for PSV-style assessment and for checking how the individual
    sites contribute to the final pole.
  </p>
  <figure>
    <img src="{vgp_rel}"
         alt="{html.escape(sheet_name)} VGP distribution"
         style="width:100%; max-width:850px; border-radius:12px;">
    <figcaption>
      Site-level VGP distribution for {html.escape(sheet_name)}.
    </figcaption>
  </figure>
</section>'''
...
def replace_site_level_content(page_text, new_card, figure_cards):
    ...
    if page_text == old:
        ...
        if h2:
            ...
            if end != -1:
                ...
                page_text = page_text[:h2.start()] + new_card + page_text[h2.end():]
        else:
    ...
    page_text = replace_section_by_id(page_text, "site-level-direction-scatter", "")
    page_text = replace_section_by_id(page_text, "site-level-vgp-distribution", "")
